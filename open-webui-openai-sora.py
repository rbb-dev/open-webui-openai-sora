...
features:
  ...
  - Supports reference media extraction from Open WebUI messages (images/videos) for future remix flows.
...
from typing import List, Dict, Any, Optional, Callable, Awaitable, Literal, Tuple, Set
import httpx
...
from starlette.responses import StreamingResponse, HTMLResponse, PlainTextResponse
...
class Pipe:
    ...
    ARTIFACT_PREFIX = '<!--sora-artifact:'
    ARTIFACT_SUFFIX = '-->'
    ARTIFACT_PATTERN = re.compile(r"<!--\s*sora-artifact:(.*?)-->")
    ZW_ARTIFACT_PREFIX = "\u2063\u2060"
    ZW_ARTIFACT_SUFFIX = "\u2063\u2061"
    ZW_BIT_ZERO = "\u200B"
    ZW_BIT_ONE = "\u200C"
    ...
    class Valves(BaseModel):
        ...
        ENABLE_SEED: bool = Field(
            default=False,
            description="If true, allow a deterministic seed parameter when requested by the user.",
        )
        DEFAULT_SEED: Optional[int] = Field(
            default=None,
            description="Optional default seed for reproducible videos when ENABLE_SEED is true.",
        )
    ...
    def __init__(self):
        ...
        if not self.allowed_resolutions:
            self.allowed_resolutions = {"720x1280", "1280x720"}
    ...
    def _build_openai_client(self) -> OpenAI:
        ...
        client_kwargs = {"api_key": self.valves.API_KEY}
        if base_url:
    ...
    async def emit_status(
        ...
        if emitter and not show_in_chat:
    ...
    def _decode_hidden_artifacts(self, text: str) -> List[Dict[str, Any]]:
        ...
        for match in self.ARTIFACT_PATTERN.findall(text):
            ...
            try:
                ...
                logger.error("Failed to decode legacy hidden artifact: %s", exc)
                continue
    ...
    async def _extract_media_from_message(self, message: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        async def _append_media_from_url(url: str):
            ...
            if url.startswith("data:"):
                ...
                if len(parts) == 2:
                    ...
                    data = parts[1]
                    media.append({"mimeType": mime_type, "data": data})
                return
        ...
        if isinstance(content, list):
            ...
            for mime_type, data in re.findall(r"!\[[^\]]*\]\(data:([^;]+);base64,([^)]+)\)", content):
                media.append({"mimeType": mime_type.lower(), "data": data})
    ...
    async def _fetch_remote_media(self, url: str) -> Optional[Dict[str, str]]:
        ...
        if not (mime_type.startswith("image/") or mime_type.startswith("video/")):
            logger.error(f"Unsupported remote media type '{mime_type}' from {url}. Skipping.")
            return None
    ...
    def _iter_response_body(self, response: Any):
        iter_bytes = getattr(response, "iter_bytes", None)
        if callable(iter_bytes):
            return iter_bytes(self.DOWNLOAD_CHUNK_SIZE)
        read = getattr(response, "read", None)
        if callable(read):
            def _reader():
                while True:
                    chunk = read(self.DOWNLOAD_CHUNK_SIZE)
                    if not chunk:
                        break
                    yield chunk
    ...
    def _build_help_markdown(self) -> str:
        ...
        lines.append("")
        lines.append("Sora only renders three clip lengths: **4 seconds**, **8 seconds**, or **12 seconds**. Ask for any other number and we pick the closest of those three automatically.")
        ...
        lines.append("If that earlier video has expired (OpenAI clears them after ~1 hour), we’ll let you know so you can regenerate it first.")
        lines.append("")
...
<html>
  ...
  <body>
    ...
    async def _analyse_prompt_with_task_model(
        ...
        if not self.valves.TASK_MODEL_ENABLED or not prompt:
            return self._get_fallback_parameters(prompt, has_media)
...
{additional_context}
Return ONLY one JSON object with this shape (omit optional keys when not used):
{{
...
2. seconds: Infer duration from the text (ranges are fine; pick one value). If nothing is specified, use {self.valves.DEFAULT_DURATION}. Any positive integer is okay; the backend snaps to 4/8/12 seconds automatically.
...
6. seed: include an integer only when the user requests determinism or names a seed (e.g., "seed 42"); omit it otherwise.
7. variations: include an integer 1..{self.valves.MAX_VARIATIONS} only when the user asks for multiple takes in the same request."""
        try:
            ...
            if not resolved_user:
                raise RuntimeError("Unable to resolve user for task model call")
            form_data = {
                "model": task_model,
    ...
    def _validate_task_model_params(
        ...
        if seed is not None and getattr(self.valves, "ENABLE_SEED", False):
            ...
            validated["seed"] = int(self.valves.DEFAULT_SEED)
        # optional variations
    ...
    def _get_fallback_parameters(
        ...
        intent = "remix" if has_media else "generate"
        duration = self.valves.DEFAULT_DURATION
        if duration < 1:
            duration = 1
        decision = self.resolution_policy.choose_size(
    ...
    def _resolve_task_model(
        ...
        def _extract(source: Any) -> Optional[str]:
            ...
            for key in ("task_model", "task_model_id", "task_model_name", "taskModel"):
                ...
                if isinstance(value, str):
                    ...
                    if value:
                        return value
            return None
        for candidate_source in (body, body.get("metadata") if isinstance(body, dict) else None):
    ...
    async def pipe(
        ...
        async def handle_request() -> Dict[str, Any]:
            try:
                ...
                dynamic_params["duration_seconds"] = validated_duration
                intent = (dynamic_params.get("intent") or "generate").lower()
                if intent not in ("generate", "remix"):
                    intent = "generate"
                dynamic_params["intent"] = intent
                seconds_str = self._map_duration_to_sora_seconds(dynamic_params["duration_seconds"])
                request_label = requested_duration_display if requested_duration_display is not None else dynamic_params["duration_seconds"]
                logger.info("User requested %ss, sending Sora seconds='%s'", request_label, seconds_str)
                if intent == "remix":
                    media = []
                    image_media = []
                    video_media = []
                ...
                if "seed" in dynamic_params:
                    extra_body["seed"] = str(dynamic_params["seed"])
                reference_filename = ""
                reference_mime_type = ""
                if intent == "generate" and has_images:
                ...
                async with self._openai_client() as client:
                    ...
                    if remix_video_id:
                        try:
                            ...
                            friendly = (
                                "That earlier video has already expired (OpenAI clears them about an hour after you generate them). "
                    ...
                    async def run_single_job(variation_index: int) -> Dict[str, Any]:
                        ...
                        await self.emit_status(
                            f"{variation_label}Submitting {action_word} video job ({dynamic_params['duration_seconds']}s @ {dynamic_params['size']})...",
                            emitter=__event_emitter__,
                        )
